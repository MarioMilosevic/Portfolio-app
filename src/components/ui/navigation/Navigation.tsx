...
const Navigation = ({ activeLink, setActiveLink }: NavigationProps) => {
  ...
  return (
    <nav className="navbar bg-neutral text-primary-content flex absolute md:fixed z-30 top-0 left-0">
      ...
      />
      <ul
        className={`absolute origin-top bg-neutral md:origin-center w-full md:w-auto ${
          navigationOpen ? "scale-y-100" : "md:scale-y-100 scale-y-0"
        } transition-all duration-300 top-0 md:top-0 flex flex-col left-0 p-2 gap-1 md:gap-0 md:p-0 md:flex-row md:absolute md:bottom-1/2 md:translate-y-1/2 md:left-1/2 md:-translate-x-1/2`}
      >
